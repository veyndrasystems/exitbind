...
```sh
curl -fsSL https://raw.githubusercontent.com/veyndrasystems/exitbind/v0.23.1-rc.2/install.sh | sh
...
The v0.23.1-rc.2 release candidate targets Linux x86_64 and native macOS on Apple
...
New checked runs in the `v0.23.1-rc.2` release candidate use run-event format 6. The
...
```sh
curl -fsSL https://raw.githubusercontent.com/veyndrasystems/exitbind/v0.23.1-rc.2/install.sh | EXITBIND_VERSION=v0.23.1-rc.2 sh
...
```text
dotagents --project add veyndrasystems/exitbind --ref v0.23.1-rc.2
```
...
```json
{
  "$schema": "https://raw.githubusercontent.com/veyndrasystems/exitbind/v0.23.1-rc.2/schema/exitbind-harness-manifest.schema.json",
  "version": 1,
