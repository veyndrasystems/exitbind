...
The v0.19.0 stable release targets Linux x86_64 and native macOS on Apple
...
New checked runs in the `v0.19.0` stable release use run-event format 6. The
...
```sh
curl -fsSL https://raw.githubusercontent.com/veyndrasystems/exitbind/v0.19.0/install.sh | EXITBIND_VERSION=v0.19.0 sh
...
before this feature cannot self-notify, so install `0.19.0` once to enable
...
```text
dotagents --project add veyndrasystems/exitbind --ref v0.19.0
```
...
```json
{
  "$schema": "https://raw.githubusercontent.com/veyndrasystems/exitbind/v0.19.0/schema/exitbind-harness-manifest.schema.json",
  "version": 1,
