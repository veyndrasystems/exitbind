...
The v0.21.0 stable release targets Linux x86_64 and native macOS on Apple
...
New checked runs in the `v0.21.0` stable release use run-event format 6. The
...
```sh
curl -fsSL https://raw.githubusercontent.com/veyndrasystems/exitbind/v0.21.0/install.sh | EXITBIND_VERSION=v0.21.0 sh
...
```text
dotagents --project add veyndrasystems/exitbind --ref v0.21.0
```
...
An install or update since `v0.21.0` keeps the replaced binary in one hidden
...
```json
{
  "$schema": "https://raw.githubusercontent.com/veyndrasystems/exitbind/v0.21.0/schema/exitbind-harness-manifest.schema.json",
  "version": 1,
