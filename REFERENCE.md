...
```sh
curl -fsSL https://raw.githubusercontent.com/veyndrasystems/exitbind/v0.24.0-rc.6/install.sh | sh
...
The v0.24.0-rc.6 release candidate targets Linux x86_64 and native macOS on Apple
...
New checked runs in the `v0.24.0-rc.6` release candidate use run-event format 8. The
...
```sh
curl -fsSL https://raw.githubusercontent.com/veyndrasystems/exitbind/v0.24.0-rc.6/install.sh | EXITBIND_VERSION=v0.24.0-rc.6 sh
...
```text
dotagents --project add veyndrasystems/exitbind --ref v0.24.0-rc.6
```
...
```json
{
  "$schema": "https://raw.githubusercontent.com/veyndrasystems/exitbind/v0.24.0-rc.6/schema/exitbind-harness-manifest.schema.json",
  "version": 1,
