...
| Observed check | v4/v5 source evidence from executing the frozen command in ProductRoot | It records an exit or signal locally; it still needs review and acceptance. |
| Reported check | Caller-supplied result bound to the frozen command | It is the only check route in v3 and one permitted route in v4/v5; it records what was reported, not proof that the caller ran the command. |
