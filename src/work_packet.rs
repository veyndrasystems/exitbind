//! Residual continuation packets: projection from canonical run state,
...
//! reprojects canonical state and compares; it does not trust submitted
//! `stillValid` or `doNotRepeat` claims and never executes packet content.
...
pub(crate) fn project(
    ...
    let packet = project_from(work, &view, &status, next);
    // One coherent snapshot: refuse to hand out facts stitched across a
    // concurrent ledger write.
    if head(&run::inspect(loaded, ledger)?) != head(&view) {
        return Err("work changed while its packet was projected; request it again".into());
    }
    Ok(packet)
}
...
fn project_from(work: &str, view: &Value, status: &Value, next: &Value) -> Value {
    ...
    {
        remaining
            .push(json!({"obligation": target["kind"], "requirementId": target["requirementId"]}));
    ...
    let input_bound = view["events"][0]["version"].as_u64() >= Some(6);
    json!({
        ...
        "snapshot": {
            ...
            "inputsSha256": if input_bound { view["inputsSha256"].clone() } else { Value::Null },
        },
        ...
        "remaining": remaining,
        "next": next["action"],
        "humanHelp": help(work, view, &targets, next, review, &remaining),
        "doNotRepeat": do_not_repeat,
        "invalidation": {
            "rule": "reuse only when the ledger head, exact subject, and tested inputs remain current",
...
fn help(
    ...
    let (what, actor, summary, cmd, owner) = if view["status"] != "running" {
        ...
        (
            format!("Work ended as {status}."),
            "none",
            "nothing remains in this run".to_owned(),
            Value::Null,
            if status == "blocked" {
                "required"
            } else {
...
/// Structure, consistency, and applicability in that order. Only `usable`
/// permits skipping the listed work; everything else returns the fresh packet.
pub(crate) fn validate(
    ...
    let fresh = project(loaded, work, ledger, next)?;
    let (result, reason) = judge(work, packet, &fresh);
    ...
    Ok(json!({
        ...
        "packet": if usable { Value::Null } else { fresh },
    }))
}

fn judge(work: &str, packet: &Value, fresh: &Value) -> (&'static str, &'static str) {
    const CANNOT: &str = "cannot_establish_applicability";
    let Some(object) = packet.as_object() else {
        return (CANNOT, "packet_not_an_object");
    };
    if packet["version"].as_u64() != Some(PACKET_VERSION) {
        return (CANNOT, "unsupported_packet_version");
    }
    ...
    {
        return (CANNOT, "unsupported_required_semantics");
    }
    let shaped = packet["work"].is_string()
    ...
    if !shaped {
        return (CANNOT, "malformed_packet");
    }
    if packet["work"] != work {
        return (CANNOT, "different_work");
    }
    if fresh["snapshot"]["inputsSha256"].is_null() {
        return (CANNOT, "tested_inputs_not_bound");
    }
    if packet["snapshot"]["inputsSha256"] != fresh["snapshot"]["inputsSha256"] {
        return ("refresh_required", "tested_inputs_changed");
    }
    if packet["snapshot"]["headEventSha256"] != fresh["snapshot"]["headEventSha256"]
        || packet["snapshot"]["eventCount"] != fresh["snapshot"]["eventCount"]
    {
        return ("refresh_required", "ledger_advanced");
    }
    if packet["stillValid"] != fresh["stillValid"] || packet["doNotRepeat"] != fresh["doNotRepeat"]
    {
        return ("refresh_required", "claims_disagree_with_state");
    }
    ("usable", "current")
}
...
mod tests {
    ...
    fn fresh() -> Value {
        json!({"version": 2, "work": "w", "snapshot": {"eventCount": 3, "headEventSha256": "h", "inputsSha256": "i"},
               "stillValid": [{"evidence": "current_check"}], "doNotRepeat": ["passed_check"]})
    }

    #[test]
    fn judge_refuses_every_non_current_packet_and_accepts_the_matching_one() {
        let fresh = fresh();
        assert_eq!(judge("w", &fresh, &fresh), ("usable", "current"));
        ...
        let cases: [(&str, Mutation, &str); 7] = [
            ...
            ("w", |p| p["stillValid"] = json!("x"), "malformed_packet"),
            ("other", |_| {}, "different_work"),
            (
                "w",
        ...
        for (work, mutate, reason) in cases {
            let mut packet = fresh.clone();
            mutate(&mut packet);
            let (result, actual) = judge(work, &packet, &fresh);
            assert_ne!(result, "usable", "{reason}");
        ...
        let mut optional = fresh.clone();
        optional["futureHint"] = json!(true);
        assert_eq!(judge("w", &optional, &fresh).0, "usable");
    }
}
