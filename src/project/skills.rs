...
pub(crate) fn activate(control: &Path, coffee: bool) -> Result<(), String> {
    ...
    for destination in destinations {
        ...
        crate::project::managed_files::ensure_managed_directory(control, directory)?;
        if destination.state == SkillRefreshState::Created {
            crate::project::managed_files::write_exclusive(
                &destination.path,
...
pub(crate) fn refresh(control: &Path, coffee: bool) -> Result<Vec<SkillRefreshStatus>, String> {
    ...
    for destination in &destinations {
        match destination.state {
            SkillRefreshState::Created => crate::project::managed_files::write_exclusive(
                &destination.path,
                destination.content.as_bytes(),
            )?,
    ...
    Ok(destinations
        ...
        .map(|destination| SkillRefreshStatus {
            ...
            embedded_sha256: crate::evidence::hash::text(&destination.content),
        })
        .collect())
}
...
fn inspect_observation(
    ...
    SkillObservation {
        ...
        optional,
        state,
        embedded_sha256: crate::evidence::hash::text(content),
        observed_sha256,
    }
}
...
fn inspect_observed_bytes(
    ...
    match crate::project::path::secure_bytes_observation(control, relative, "managed project skill")
    {
        crate::project::path::SecureBytesResult::Bytes(bytes) => {
            let observed_sha256 = crate::evidence::hash::bytes(&bytes);
        ...
        crate::project::path::SecureBytesResult::Absent(_) => (SkillObservationState::Absent, None),
        crate::project::path::SecureBytesResult::Unsafe(_) => (SkillObservationState::Unsafe, None),
