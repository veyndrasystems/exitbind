...
use crate::{config::Loaded, evidence::hash, project::managed_files};
...
pub(crate) fn prepare_paths(loaded: &Loaded, apply: bool) -> Result<Value, String> {
    ...
    let desired = crate::project::layout_types::control_dirs()
        .into_iter()
        .map(|relative| ("control", &loaded.control_root, relative))
        .chain(
            crate::project::layout_types::state_dirs()
                .into_iter()
    ...
    let canonical = crate::evidence::harness::canonical_path(loaded);
    ...
    let state_targets = directories
        ...
        .map(|operation| operation.path.as_path())
        .collect::<Vec<_>>();
    crate::project::git_preflight::refuse_tracked_targets(&loaded.control_root, &control_targets)?;
    crate::project::git_preflight::refuse_tracked_targets(&loaded.state_root, &state_targets)?;
...
pub(crate) fn run(loaded: &Loaded, apply: bool) -> Result<Value, String> {
    ...
    for (name, agent) in &loaded.agents {
        ...
        let target = format!(
            "{}/{suffix}",
            crate::project::layout_types::CANONICAL_AGENTS_DIR
        );
        grouped
            .entry(source)
    ...
    crate::project::git_preflight::refuse_tracked_targets(&loaded.control_root, &targets)?;
