...
pub(crate) fn fingerprint(loaded: &crate::config::Loaded) -> Result<String, String> {
    ...
    let mut paths = match crate::git_preflight::worktree_root(root)? {
    ...
    for relative in paths {
        ...
        let entry = match fs::symlink_metadata(&full) {
            ...
            Ok(meta) if meta.file_type().is_symlink() => {
                ...
                json!([text, "symlink", crate::hash::text(target)])
            }
            Ok(meta) if meta.is_file() => json!([text, "file", crate::hash::file(&full)?]),
            Ok(_) => continue,
        };
        entries.push(entry);
    }
