...
pub use crate::project::path::{absolute, file, rel};
...
pub struct Loaded {
    ...
    pub mode: crate::project::layout_types::Mode,
...
pub fn validate(config: &Value) -> Vec<String> {
    ...
    crate::memory::policy::validate(&config["memory"], &mut errors);
...
fn validate_agent(name: &str, value: &Value, errors: &mut Vec<String>) {
    ...
    for field in MEMORY_SCOPE_FIELDS {
        if agent
            .get(*field)
            .is_some_and(|value| !crate::memory::policy::valid_scope_array(value))
        {
            errors.push(format!(
