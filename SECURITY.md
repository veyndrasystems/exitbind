...
In the `v0.24.0-rc.5` release candidate, `run observe-check` executes only the frozen command
...
binding fails. Observation uses a bounded deadline, runs outside the ledger
lock, terminates its POSIX process group and reaps the command process on
...
run; v3 supports caller-reported `run record-check` only. The `v0.24.0-rc.5` release candidate
